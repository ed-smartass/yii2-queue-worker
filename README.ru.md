...
Если воркер неожиданно завершается (не вручную и не по сигналу), behavior запускает новый процесс воркера взамен. Чтобы избежать crash-loop, перезапуск происходит только если воркер проработал не менее `minRestartUptime` секунд (по умолчанию `10`). Воркеры, умершие быстрее, логируются как warning и оставляются на обработку health-check cron.

Счётчик перезапусков через процессы намеренно не ведётся: каждый новый воркер — свежий PHP-процесс со своей памятью. Для более жёсткой защиты от crash-loop используйте внешний supervisor (`systemd`, `supervisord`) или cron-команду `worker/check`, которая находит мёртвые записи в БД и перезапускает их.
