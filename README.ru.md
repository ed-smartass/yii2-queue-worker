...
Процессы воркеров отслеживаются в таблице базы данных: PID, имя компонента, таймстемп heartbeat, текущий ID задачи. Расширение поддерживает graceful shutdown через POSIX-сигналы (Ctrl+C), in-process авто-рестарт с защитой от crash-loop и консольную команду проверки здоровья для мониторинга через cron.
...
| `minRestartUptime` | int | `10` | Минимальное время жизни воркера (сек) перед авто-рестартом после крэша |
...
Если воркер неожиданно завершается (не вручную и не по сигналу), behavior запускает новый процесс воркера взамен. Чтобы избежать crash-loop, перезапуск происходит только если воркер проработал не менее `minRestartUptime` секунд (по умолчанию `10`). Воркеры, умершие быстрее — а также воркеры с отсутствующим или невалидным `started_at` — логируются как warning и **не** перезапускаются автоматически: запись в БД удаляется, и такие воркеры нужно поднимать внешним supervisor-ом (`systemd`, `supervisord`) или повторным вызовом `start()` вручную.

Обратите внимание: cron-команда `worker/check` восстанавливает воркеры, чей процесс умер **без** вызова `onWorkerStop()` (жёсткий kill, segfault). Она не восстанавливает воркеры, которые пропустил crash-loop-гард выше, потому что их записи удаляются как часть обычного stop-события. Если нужна жёсткая in-process защита от crash-loop — используйте внешний supervisor.

Счётчик перезапусков между процессами намеренно не ведётся: каждый новый воркер — свежий PHP-процесс со своей памятью.
